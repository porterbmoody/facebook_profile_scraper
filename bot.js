...
class Bot {
    constructor(response) {
        ...
        this.meta_data = {
            ...
            "detail_tabs": "[class='x193iq5w xeuugli x13faqbe x1vvkbs x1xmvt09 x1lliihq x1s928wv xhkezso x1gmr53x x1cpjm7i x1fgarty x1943h6x xudqn12 x3x7a5m x1f6kntn xvq8zen x1s688f xi81zsa']"
        }
    }
    ...
    async read_existing_data() {
        ...
        try {
            ...
            console.log(`number of existing profiles: ${this.existing_profile_data['profile_name'].length}`);
        } catch (error) {
    ...
    async view_all_members() {
        ...
        while (continue_scrolling) {
            ...
            if (newUrls.size >= this.response['profiles_to_scrape']) {
                console.log(`number of new urls found is greater than input number of profiles to scrape`);
            ...
            if (isAtBottom) {
                console.log(`scrolled to bottom of page, stopping scroll`);
                continue_scrolling = false;
            } else {
    ...
    async scrape_profiles() {
        try {
            ...
            console.log(`profiles to scrape: ${this.group_profile_urls.length}`);
            console.log(`in hours: ${this.response['hours_to_scrape']}`);
            console.log('opening group profile urls');
            for (const [index, group_profile_url] of this.group_profile_urls.entries()) {
                // try {
                // const group_profile_url = 'https://www.facebook.com/ContessaCleaningCompany';
                ...
                console.log(`[${progressBar}] ${progress.toFixed(2)}% | ${pagesScraped}/${totalProfiles}`);
            ...
            // console.log(`Scraping completed. Total profiles scraped: ${pagesScraped}`);
        } catch (error) {
